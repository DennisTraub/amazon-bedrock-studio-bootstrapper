...
class BedrockStudioBootstrapper:
    ...
    def _process_configuration(self, config):
        ...
        if (
            ...
            logger.info(
                f"- OpenSearch configuration: Enabled. "
                f"OpenSearch domain ID: '{self._config.opensearch_config.domain_id}'"
            )
                        ...
                        "Condition": {
                            "StringEquals": {
                                "iam:PermissionsBoundary":
                                    f"arn:aws:iam::{account_id}:policy/{self._permission_boundary_policy_name}",
        ...
        return json.dumps(
            {
                ...
                "Statement": [
                    ...
                    {
                        "Sid": "Allow Amazon Bedrock to encrypt and decrypt Agent resources on behalf of authorized"
                               "users",
                        "Effect": "Allow",
